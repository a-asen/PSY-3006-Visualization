...
sleep_data2 |> 
  ...
  geom_ribbon(mapping=aes(ymin=(daylight_length_h/reduction)+additional_start,
                          ymax=24/reduction+additional_start, 
                          xmin=week, xmax=week), fill="black", alpha=.3)+
  stat_summary(mapping=aes(x=cumulative_days+week/365), 
               fun.data=mean_cl_normal, geom="pointrange", color="red")+
  geom_smooth()+
  #stat_summary(fun = mean,geom="line", color="red", size=.5)+
  scale_x_continuous(breaks=seq(0,365,30), labels = c(0, unique(sleep_data2$date_m_name)))+
  theme_bw()+
...
```{r circle sum week aesthetic change+season}
reduction <- 6
additional_start <- 6
sleep_data2 |> 
  group_by(week) |> 
  ...
  geom_ribbon(mapping=aes(ymin=(daylight_length_h/reduction)+additional_start,
                          ymax=24/reduction+additional_start, 
                          xmin=week, xmax=week), fill="black", alpha=.3)+
  #geom_rect(mapping=aes(xmin=1,xmax=13,ymin=6,ymax=10), fill="#208aaa", alpha=.1)+
  annotate("rect",xmin=1,xmax=13,ymin=6,ymax=10, alpha=.4, fill="#208aaa")+
  annotate("rect",xmin=13,xmax=26,ymin=6,ymax=10, alpha=.4, fill="#feb612")+
  annotate("rect",xmin=26,xmax=39,ymin=6,ymax=10, alpha=.4, fill="#973905")+
  annotate("rect",xmin=39,xmax=52,ymin=6,ymax=10, alpha=.4, fill="#005d72")+
  stat_summary(fun.data=mean_cl_normal, geom="pointrange", color="red")+
  geom_smooth()+
  #stat_summary(fun = mean,geom="line", color="red", size=.5)+
  scale_x_continuous(breaks=seq(0,52,2))+
...
In comparison, the circle plot produce a novel and interesting way of plotting data. 
