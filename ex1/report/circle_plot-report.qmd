...
In this display, the data points are the main focus of the plot, but also illustrate the amount of daylight throughout the year. One design feature is to reduce by an even number. Thus each of the lines in the plot can give valuable information regarding the relationship between the daylight hours throughout the year. By using a division by 6, the middle line of the daylight length indicate half the day. Such that we can see that by day 75 and 270 (see the dashed lines) half the day contains sunlight and the other half contains no sunlight. We see the same feature in the first plot with daylight length, but which is harder to see in the second plot. On the other hand, it can appear as though the data is a symbol instead of indicating sunlight length. 

To further enhance the difference between the data point, we can further transform the daylight visualization. Due to the coord_polar implementation, it will reduce the plot to the smallest value on the plot. In the first circle plot implementation, the smallest value was close to 6, and so the plot started at 6 (at the very center). By replacing geom_area with geom_ribbon, we can limit our daylight visualization to start at some value above 0, since there is no valuable data presented before the 6th hour. With this, we can also retain the contour of the year-round daylight length, in similar ways as we have done previously. 

```{r circle some middle path}
reduction <- 4  # (24+6*4/10)
additional_start <- 4
sleep_data |>
  ggplot(aes(x=cumulative_days, y=sleep_amount))+
...
In this plot, it is easier to discern the difference between each data point, while also retaining the changes in daylight hours throughout the year. It is, nonetheless, important to point out that it is not accurate to present daylight hours in this way. It is more accurate to say the proportion of daylight throughout the year. However, it is, as before, important to note that the illustration of daylight is based on a 24 hour clock, but transformed to fit within the 6 hour span of the plot.  

Another way of visualizing the sleep amount is to visualize the sleep time and wake time. This, however, might be more difficult as the span ranges from around 5 hours to above 24 hours (indicating going to sleep later than 24, which should be transformed to 0). Nevertheless, it is worth to investigate how the visualization changes. 
