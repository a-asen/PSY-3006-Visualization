...
(Lower categlory indicate more blanking)
In the above plot, we can see a very similar pattern to the previous plots. It is not entirely clear whether there is any difference between the categorical responses (there is a lot of overlap). Nevertheless, we can see that the categorical response 3 and 4 have slighly wider distributions as well as less of a normal distribution - it seems to have two peaks. 
