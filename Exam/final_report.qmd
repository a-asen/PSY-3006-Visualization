...
In this course, I utilize the ggplot2 package in R to visualize data. The ggplot2 package builds on the foundation that "The Grammar of Graphics" lay. This is done to learn how to visualize data in R using the "grammar" of visualizing. The benefit of this framework is that most things should be able to be visualized with a solid understanding of the package. Moreover, the course requires the use of version control software, like Git, and reporting tools like Quarto. Git makes it possible to store a version of the code. This makes it possible to go back in time (so to speak) to fix bugs that might have arisen during coding. Moreover, it is an important tool in collective coding efforts, as it can be incredibly difficult to coordinate code from different people. Quarto is a reporting tool built on Rmarkdown, which is again built from Markdown. Markdown is a lightweight markup language that is used to structure the format of a text file with simple symbols. For instance, the title "course goal" is started with two hashtags (##) which create a "title heading level 2" (three hashtags would create a level 3 heading - see "Circle visualization"). RMarkdown incorporates R code with the basic markdown structure, such that the text file can be easily structured and include R code and its output. Quarto builds on RMarkdown by incorporating its own syntax and improving certain features. For instance, by making it possible to do inline codes using the "\`". Inline code makes it possible to implement code within a line of text. This is particularly handy if you are doing data analysis, but change one aspect which may require the rewriting of numbers. 


## Circle visualization

A typical plot has an x and y-axis, often portrayed in a rectangular way. However, it is also possible to visualize data circularly. The circle plot changes the normal portrayal of the rectangular plot by *connecting back to itself*. Plotting in this way is not widespread because of obvious problems with visualizing in this way, as we will see throughout this exploration. However, there are some areas that can benefit by displaying information circularly. The best use-cases for circle plots are in data that naturally follows a circular pattern. This can, for instance, data over seasons, days and hours. All these aspects typically follow a recurring aspect. Indeed, Wilkinson's (2006, page 213) notes that circle plots can be used for displaying seasonal data. In the first part, consisting of two exercises, of this course, I will visualize data in a circular way.


### Ex1: Seasonal sleep

The first part relates to visualizing fictive seasonal data circularly. The data pertains to sleep data of a fictive participant that sleeps longer during the winter period and shorter during the summer period, with a lot of variation throughout the year. The exercise explores how to visualize the sleep data in a simple and effective manner. Through that exploration, I found that visualizing all the data points is a bit difficult throughout a year. It leads to problems in interpreting what data point was related to which day. However, by summarizing the data over each week and adding an error bar, we can visualize sleep in a more intelligible way. Although visualizing circularly can yield more pleasing figures, the loss of accurate data representation might not be worth it. Indeed, visualizing in a normal (rectangular way) yielded a greater overview of the distribution throughout the year. In this plot, we could easily see the changes in sleep amount over time. However, the relationship between seasons may be less intuitive. Hence, there are advantages and disadvantages to visualizing data through a season using circle plots. To further explore the utility of circle plots, I turn to visualizing the sleep time of participants in a sleep experiment. 


### Ex2: Sleep times

For the second exercise, I investigated how to visualize sleep times between two conditions. In one of the conditions, participants were sleep-deprived, while in the other they slept normally. For the sleep deprivation condition, participants were asked to go to sleep 3 hours later than when they typically go to bed (i.e., under normal sleep). From this, I expected the sleep times to be obviously different between the conditions. The idea was to visualize sleep time in relation to the hours of a circle, pertaining to a standard 12-hour clock. The height of the points related to the number of hours slept. In this way, we should see those who went to sleep during the normal sleep condition to go to sleep earlier and have higher values - meaning they slept longer. Furthermore, I investigated how it would be to visualize on a 24-hour clock. Both have some advantages and disadvantages, but I ended up using a 24-hour clock split by 12 hours, from 18 to 6. This was because the relevant time window was within that time, and would correspond to a 12-hour clock in a meaningful manner (24 was at the top). Nevertheless, some suggestions were to use an am/pm split, and possibly include wake time. As for the latter, I did investigate a 24-hour clock including wake times, which has some merit, but led to fewer details in the sleep times. Moreover, both the relevant informative places are given by sleep time and sleep length - you can calculate wake times. Nevertheless, seeing that wake time is not significantly different between the conditions is easier than interpreting it from the sleep times. 



## Multivariate visualization

For the two latter exercises (ex3 and ex4), I focus on multivariate visualization. That is visualizing with multiple variables in one plot. The reason to do this is that you can gain a more intuitive understanding of how the variables relate. Moreover, it can indicate patterns and how two variables are related to each other. In this case, I have utilized data from a mind wandering (MW) study with the variables MW, approximate entropy (AE) and behavioural variability (BV). Mind wandering is reported on a 4-point Likert scale, lower values indicating more mind wandering. AE is a continuous score of the sequence randomness that participants generate, higher values indicate more randomness - in this case, better. Furthermore, BV is also a continuous variable that relates to participants' consistency in timing of responses in any sequence length. The first part (ex3) investigated visualization of two continuous variables, while the second part (ex4) investigated a continuous and a categorical variable.


### Ex3: Continuous-continuous

For this exercise, I utilized AE and BV by visualizing their joint distribution. Firstly, I was investigating different ways of visualizing multiple distributions, and ended up with a joint distribution using a 2D density plot. From how the variables are defined, we may expect that when BV is low, that AE is also high - at least that would be the perfect outcome. Then, as BV increases, AE should decrease. This would then be related to on-task thoughts and off-task thoughts respectively. Through the investigation, no clear pattern emerged, but higher values of BV were typically found with lower values of AE, and higher values of AE were typically found with lower values of BV. Thus, visualizing these variables in a joint distribution does indicate that AE and BV exhibit the expected pattern, although this is not crystal clear. 


### Ex4: Categorical-continuous

For the last visualization, I investigated how AE were distributed over different MW responses. This might be a more meaningful split, as we expect higher AE scores when participants report to be on-task as compared to when they are off-task. The visualization is slightly different from the joint distribution of the previous exercise. In this exercise, I also started by investigating different ways of visualizing and settled on the overlapping distribution by using *ggridges*. Ggridges makes it easy to visualize distributions over categorical values and closely overlapping them - which was a problem otherwise. The last AE figure did not obviously indicate that AE was increased during on-task reports (3-4), but seemed to have slightly higher scores. Lastly, I quickly checked the distribution of BV over MW, and BV over mind blanking (MB). Mind blanking is reported on a 4-point Likert scale relating to whether participants' mind wandered about content (4) or nothing (blank - 1).
