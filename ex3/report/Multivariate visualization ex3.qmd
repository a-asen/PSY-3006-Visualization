...
With this library we get a more neat visualization of both the individual and joint distribution. Moreover, using a coloured 2d density distribution we can more easily see how the data is distributed. Indeed, we can here see that BV seems to have two areas of observations as noted above. Moreover, it seems like AE has a wider distribution, which might be beneficial if we want to observe a difference between cognitive states.  

With this, I will try and make the plot more neat with aesthetic changes. 
...
Even thought it might not be obvious from the plot above, data points outside the range that is displayed are remove. However, the distribution of the remaining data points are valid, but does not capture all the data points anymore (compare to the above or below). 

Thus, if we wish to visualize the distribution of the data, we should include all the data points we can. For this purpose I will stick with the *coord_cartesian* scale, and increase the size of the side plots to improve its visibility.
...
neat_ae.bv +  
  ...
  geom_ysideline(aes(x=1), linetype = 2, alpha = .5)+ 
    # No idea why these two latter do not show a dashed line...
...
However, the problem remain. It is still hard to see the exact distributions of the densities on the side. This could plausibly be fixed by re-scaling the side plots. However, I have not been able how to do this, even though plausible functions should be able to do it (e.g., scale_xsidey_continous, e.g., https://rdrr.io/cran/ggside/man/ggside-scales-continuous.html). I assume that *coords_cartesian* supersede scale limitations, and thus we are unable to change the scale of the side distributions - see below for an attempt. 
...
Another way of visualizing the data distribution is to use 3 dimensional (3D) plots. 
...
plot_ly(x=~density_data$x, y=~density_data$y, z=~density_data$z, type="surface") |>
  layout(
    scene = list( 
      camera = list(
        eye = list(x=2,y=-1.5,z=2))))
  
```

There can arise a variety of problem when visualizing things in 3D. However, it can also be uniquely informative - especially if they are interactive. The latter of which is a worthy point, from my perspective, as I can rotate the plot within R. However, granted the reader is reading from a pdf file (or otherwise), the interactive mode is not possible. Nevertheless, this plot do not use *ggplot*, since ggplots do not have a way to visualize things in 3D. For this plot, I have used *plotly*, which is a library that can visualize data in 3D. However, I will not explore plotly in depth as it is beyond the scope of ggplot, but is nevertheless interesting to point out. 
