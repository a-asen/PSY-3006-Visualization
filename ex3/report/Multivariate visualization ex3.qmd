...
especially if they are interactive. The latter of which is a
worthy point, from my perspective, as I can rotate the plot
within R. However, granted the reader is reading from a pdf
file (or otherwise), the interactive mode is not possible.
Nevertheless, this plot do not use *ggplot*, since ggplots do
...
With this, I can explore some basic features of the different
variables. Moreover, there are different ways of visualizing
multivariate

```{r}
trans_mw |> 
  ggplot(aes(x=scale(zlogbv), y=scale(zlogapen)))+ 
  geom_hex()
  geom_density()
  geom_contour()
  
trans_mw |> 
  ggplot(aes(x=scale(logbv), y=scale(apen)))+
  geom_point()
