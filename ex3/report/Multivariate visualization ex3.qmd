...
suppressWarnings(da_pro(mw_data, nback=25)) -> trans_mw
...
in relation to each othe, over a joint distribution space,
...
We can now start plotting the data. There are a only a couple
of ways one can visualize the distribution of data over a
variable. This could be with a histogram or a density
distribution. A histogram summarize the data according to the
number of occurances within a certain span.
...
test, where the ranges are be more meaningful. That is, a 5
...
Indeed, in this plot we get a more neuanced view of how the
data is distributed. More specifically, we can see that there
are a lot of values close to -2. This may be because of how AE
...
normal distribution is greater than the sum of this extreme
...
Indeed, visualizing the raw AE score suggest that we see that
most values center around 0 (floor) and the ceiling. This is
...
However in these cases the relationship between the variables
may be ambiguous and underexlored. In particular, we may be
interested in how these two variables relate to each other.
This can be achieved by visualizing the joint distribution
with a 2d density plot. This can be done in multiple ways, for
instance, by simply plotting all the data points.
...
An obvious problem arise as the data points overlap and hide
each other. One solution is to add transparency so we can see
all (or mostly all) data points - and also how they overlap.
...
Nevertheless, it is not easy to gauge the number of
observations within each value. However, the plot nevertheless
reveal some interesting findings. For instance, we can see
that for the BV variable, how data points cluster around 0 but
also around 1.5 standard deviations above 0.

Alternatively, and in a similar way to histograms, we can
count the number of observations within a span. This can be
done by using hexagons:
...
variables is that when participants are focused on the task,
...
+2, and only few cases to the extreme. If anything, the
annotations I have made should be closer to the center.
...
We can see that BV have more extreme observations than AE, for
...
that for BV, data seem to gather around 0 but also around 1.5
...
deviations of BV. In a similar vein, AE have most values
clustered around 0 (because of z-scoring), but we can also see
that AE have a good amount of observations at the very bottom
...
density function, which may have lead to some changes in
number of bins.
...
are looking at the join distibution of AE and BV we can add
...
distributed. Indeed, we can here see that BV seems to have two
...
remove data points from outside of the graph. Leading to other
problems...
...
points outside the range that is displayed are remove.
However, the distribution of the remaining data points are
valid, but does not capture all the data points anymore
...
However, the problem remain. It is still hard to see the exact
...
```
![Plotly plot 2](../figs/plotly_plot2.png)

There can arise a variety of problem when visualizing things
in 3D. However, it can also be uniquely informative -
especially since these plots are interactive. That is, the plot can be rotated and zoomed within R (it is a HTML element). However, if the reader is reading through a texxt format (e.g., pdf) file (or otherwise), such an interactive exploration of the data distribution is not possible. Nevertheless, this plot do not use *ggplot*, since ggplots do
not have a way to visualize things in 3D. For this plot, I
have used *plotly*, which is a library that can visualize data
in 3D. However, I will not explore plotly in depth as it is
beyond the scope of ggplot, but is nevertheless interesting to
point out.

Thus, in summary, visualizing the joint distribution of two variables can be informative in relation to how these variables interact. It can become evident whether the distribution change depending on some variable. In this case, we would expect that higher values of AE would be associated with more consistent BV. Although, it is not entirely clear whether this is the case as there is much variation in the distributions. 
