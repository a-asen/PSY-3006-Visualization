...
neat_ae.bv +  
  ...
  geom_xsidedensity()+ 
  geom_ysidedensity()+
  scale_xsidey_continuous(expand=F, limits = c(0,1))+
  theme(ggside.panel.scale=.4) -> p_ggside
p_ggside  
```

One aesthetic change is to zoom into the main density area of the plot, which is mainly in the bottom left corner. However, doing this, has led to some cases related to the side densities. The side densities inherents the coord_cartersian feature from the ggplot object, making it wider, and thus harder to see the density that is locked between 0 and 1. I have not been able to find a solution to limit only the main plot, but no the side plots. An alternative, is to use scale_x/y with the "limits" parameter. However, this will also remove data points from outside of the graph. Leading to some problems. 
