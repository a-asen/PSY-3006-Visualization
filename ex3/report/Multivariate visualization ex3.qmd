...
We can see that BV have more extreme observations than AE, for instance. Moreover, AE seems to take great leaps for the more extreme values. Nevertheless, most of the data seems to be relatively normally distributed. Furthermore, we can also see that BV seems to have a peak around 1.5 above 0 (which is more obvious in the individual distribution and the hex plot - compare to the plots below), but that AE is nevertheless low. In a similar vein, AE seems to have some observations close to 0, where BV is nevertheless quite low. I have marked what I mean in the plot below.
