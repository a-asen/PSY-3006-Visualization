...
However in these cases the relationship between the variables may be ambiguous and underexlored. In particular, we may be interested in how these two variables related to each other. We can explore this by visualizing the join distribution with a 2d density plot. This can be done in multiple ways, for instance, we can use hexagons to count the number of occurances within each joint area.
