...
editor: 
  markdown:
      wrap: 62
---
<!--- #fontfamily: "Arial"  idk doesnt work? -->
```{r ..}
#| echo: false
library(tidyverse)
# loader script
...
s_sleep2 |>
  ...
  scale_x_continuous(breaks=seq(0,29,1), labels=c(seq(0,23,1), seq(0,5,1)), minor_breaks = F)+
  ...
  labs(x="Sleep time", y="Sleep duration", col="Condition", fill="Condition", 
       ...
       caption="Sleep duration (y) over sleep time (x). NS = normal sleep, PSD = partial sleep deprivation.
       Length of the bars represents sleep duration, and x location indicate sleep time. 
       Sleep time according to clock time displayed from 18 (evening) to 6 (morning).")
...
```{r transformation of cum_sum & vis }
sleep |>
  group_by(sleepdep) |>
  mutate(sd_length = length(sleepdep)) |> ungroup() |>
  group_by(subj, sleepdep) |>
